...
class User:
    ...
    # @classmethod
    # def get_all_results_by_user(cls,data):
    #     query = "SELECT * FROM users LEFT JOIN results ON users.id = results.user_id WHERE users.id = %(id)s"
    #     results = connectToMySQL(cls.my_db).query_db(query,data)
    #     if len(results) < 1:
    #             return None
    #     else:
    ...
    #             } 
    #             result_instance = result.Result(result_dictionary)
    #             this_user.result.append(result_instance)
    #         return this_user